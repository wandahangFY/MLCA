...
cv_resize_flags = [cv2.INTER_NEAREST,
                   cv2.INTER_LINEAR,
                   cv2.INTER_CUBIC,
                   cv2.INTER_AREA,
                   cv2.INTER_LANCZOS4,
...
#
cv_resize_flags_with_weights = {cv2.INTER_NEAREST: 1,
                                cv2.INTER_LINEAR: 1,
                                cv2.INTER_CUBIC: 1,
                                cv2.INTER_AREA: 1,
...
def RandomResize(with_weights=False):
    if with_weights:
        return random.choices(list(cv_resize_flags_with_weights.keys()), weights=list(cv_resize_flags_with_weights.values()), k=1)
    else:
